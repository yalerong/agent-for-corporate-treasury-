...
一个**会自己获取数据、自己总结规律**的资金管理智能体：把付款流水喂给它，它自动提炼出发薪日、固定付款、部门预算偏差等业务规律，产出**预算执行差异分析、滚动资金预测、外汇交易管控建议、关联方合规视图、审批流程画像**五份决策材料。

核心设计哲学一句话：**LLM 总结规律，代码执行规律**——报告里每一个数字都来自确定性引擎，可追溯、可重放。当前版本的规律提炼由确定性统计规则完成；LLM 归纳环（只读聚合摘要、产出候选规律、永不直接给数字）在路线图中。
...
- `high` → 进入引擎计算，报告尾注标注待人工批准的数量；"批准才生效"的硬门与批量审批 CLI 在路线图中
...
- [x] Phase A：数据核心闭环（入库 → 规律 → 报告），本仓库当前状态；确定性由回归网守护——`tests/cashflow` 用固定种子合成数据对全链路数字做精确断言（golden 快照），CI 每次变更必跑
